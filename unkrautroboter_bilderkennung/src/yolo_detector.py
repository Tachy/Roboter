...
if not logging.getLogger().hasHandlers():
    logging.basicConfig(
        level=getattr(config, "LOGLEVEL", logging.INFO),
...
def _mp_predict_worker(
    ...
    try:
        ...
        if (
            ...
            and (globals().get("model") is not None)
        ):
        ...
        try:
            for r in res:
                ...
                if (
                    hasattr(r, "boxes")
                    and hasattr(r.boxes, "xywh")
                    and r.boxes.xywh is not None
                ):
...
def extract_xy(results):
    ...
    if config.USE_DUMMY:
        ...
        return [(640.0, 600.0), (600.0, 550.0), (700.0, 500.0)]  # Beispielkoordinaten
    coordinates = []
    try:
        for r in results or []:
...
def process_image(image_path):
    ...
    if config.USE_DUMMY:
        ...
        if "model" not in globals() or model is None:
            logger.error(
                "[YOLO] Kein Modell verfügbar. Prüfe YOLO_MODEL_PATH oder setze USE_DUMMY=True."
            )
            return []
    ...
    if _weights_abs or True:
        ...
        p = ctx.Process(
            target=_mp_predict_worker,
            args=(
                q,
                image_path,
                _weights_abs or _weights,
                device,
                imgsz,
                conf,
                iou,
        ...
        if p.is_alive():
            try:
                ...
                pass
            logger.error(
                f"[YOLO] Inferenz-Timeout – Subprozess beendet (>{timeout_s:.1f}s)."
            )
            return []
        if p.exitcode != 0:
            dur = (time.time() - t0) * 1000.0
            logger.error(
                f"[YOLO] Inferenz-Subprozess exitcode={p.exitcode} nach {dur:.0f}ms"
            )
            return []
        try:
        ...
        if isinstance(mem_peak_kb, int) and mem_peak_kb > 0:
            ...
            try:
                ...
                logger.info(
                    f"[YOLO] RAM: max_peak={max_mb:.1f} MB (dieser Lauf: {cur_mb:.1f} MB)"
                )
            except Exception:
                pass
        if isinstance(tmp_used, int) and tmp_used > 0:
            ...
            try:
                ...
                if total_mb > 0:
                    logger.info(
                        f"[YOLO] /tmp: used={used_mb:.1f}/{total_mb:.1f} MB (max_used={max_used_mb:.1f} MB), ann_size={(ann_size or 0)/1024:.0f} KB"
                    )
                else:
                    logger.info(
                        f"[YOLO] /tmp: used={used_mb:.1f} MB (max_used={max_used_mb:.1f} MB), ann_size={(ann_size or 0)/1024:.0f} KB"
                    )
            except Exception:
                pass
        dur = (time.time() - t0) * 1000.0
